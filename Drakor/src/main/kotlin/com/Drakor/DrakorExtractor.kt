...
import com.Drakor.Drakor.Companion.cinemaOSApi
import com.Drakor.Drakor.Companion.Player4uApi
import com.Drakor.Drakor.Companion.idlixAPI
import com.Drakor.Drakor.Companion.RiveStreamAPI
...
object DrakorExtractor : Drakor() {
    ...
    suspend fun invokeVidsrccc(
        ...
        val url = if (season == null) {
            "${Drakor.vidsrcccAPI}/v2/embed/movie/$tmdbId"
        } else {
            "${Drakor.vidsrcccAPI}/v2/embed/tv/$tmdbId/$season/$episode"
        }
        ...
        val serverUrl = if (season == null) {
            "${Drakor.vidsrcccAPI}/api/$tmdbId/servers?id=$tmdbId&type=movie&v=$v&vrf=$vrf&imdbId=$imdbId"
        } else {
            "${Drakor.vidsrcccAPI}/api/$tmdbId/servers?id=$tmdbId&type=tv&v=$v&vrf=$vrf&imdbId=$imdbId&season=$season&episode=$episode"
        }

        app.get(serverUrl).parsedSafe<VidsrcccResponse>()?.data?.amap {
            val sources =
                app.get("${Drakor.vidsrcccAPI}/api/source/${it.hash}").parsedSafe<VidsrcccResult>()?.data
                    ?: return@amap
                ...
                it.name.equals("VidPlay") -> {
                    ...
                    callback.invoke(
                        newExtractorLink(
                            ...
                            this.referer = "${Drakor.vidsrcccAPI}/"
                        }
                    )
    ...
    suspend fun invokeVidsrc(
        ...
        val url = if (season == null) {
            "${Drakor.vidSrcAPI}/embed/movie?imdb=$imdbId"
        } else {
            "${Drakor.vidSrcAPI}/embed/tv?imdb=$imdbId&season=$season&episode=$episode"
        }
    ...
    suspend fun invokeXprime(
        ...
        runAllAsync(
            {
                val url = if (season == null) {
                    "${Drakor.xprimeAPI}/${servers.first()}?id=$tmdbId"
                } else {
                    "${Drakor.xprimeAPI}/${servers.first()}?id=$tmdbId&season=$season&episode=$episode"
                }
            ...
            {
                val url = if (season == null) {
                    "${Drakor.xprimeAPI}/${servers.last()}?name=$title&fallback_year=$year"
                } else {
                    "${Drakor.xprimeAPI}/${servers.last()}?name=$title&fallback_year=$year&season=$season&episode=$episode"
                }
    ...
    suspend fun invokeWatchsomuch(
        ...
        val epsId = app.post(
            "${Drakor.watchSomuchAPI}/Watch/ajMovieTorrents.aspx", data = mapOf(
                "index" to "0",
                "mid" to "$id",
        ...
        val subUrl = if (season == null) {
            "${Drakor.watchSomuchAPI}/Watch/ajMovieSubtitles.aspx?mid=$id&tid=$epsId&part="
        } else {
            "${Drakor.watchSomuchAPI}/Watch/ajMovieSubtitles.aspx?mid=$id&tid=$epsId&part=S${seasonSlug}E${episodeSlug}"
        }
        ...
        app.get(subUrl).parsedSafe<WatchsomuchSubResponses>()?.subtitles?.map { sub ->
            subtitleCallback.invoke(
                newSubtitleFile(
                    ...
                    fixUrl(sub.url ?: return@map null, Drakor.watchSomuchAPI)
                )
            )
        }
    }
    ...
    suspend fun invokeMapple(
        ...
        val url = if (season == null) {
            "${Drakor.mappleAPI}/watch/$mediaType/$tmdbId"
        } else {
            "${Drakor.mappleAPI}/watch/$mediaType/$season-$episode/$tmdbId"
        }
        ...
        val subRes = app.get(
            "${Drakor.mappleAPI}/api/subtitles?id=$tmdbId&mediaType=$mediaType${if (season == null) "" else "&season=1&episode=1"}",
            referer = "${Drakor.mappleAPI}/"
        ).text
        tryParseJson<ArrayList<MappleSubtitle>>(subRes)?.map { subtitle ->
            subtitleCallback.invoke(
                newSubtitleFile(
                    subtitle.display ?: "",
                    fixUrl(subtitle.url ?: return@map, Drakor.mappleAPI)
                )
            )
        }
    }
    ...
    suspend fun invokeVidlink(
        ...
        val url = if (season == null) {
            "${Drakor.vidlinkAPI}/$type/$tmdbId"
        } else {
            "${Drakor.vidlinkAPI}/$type/$tmdbId/$season/$episode"
        }

        val videoLink = app.get(
            url, interceptor = WebViewResolver(
                Regex("""${Drakor.vidlinkAPI}/api/b/$type/A{32}"""), timeout = 15_000L
            )
        ...
        callback.invoke(
            newExtractorLink(
                ...
                this.referer = "${Drakor.vidlinkAPI}/"
            }
        )
    }
    ...
    suspend fun invokeVidfast(
        ...
        val type = if (season == null) "movie" else "tv"
        val url = if (season == null) {
            "${Drakor.vidfastAPI}/$type/$tmdbId"
        } else {
            "${Drakor.vidfastAPI}/$type/$tmdbId/$season/$episode"
        }

        val res = app.get(
            url, interceptor = WebViewResolver(
                Regex("""${Drakor.vidfastAPI}/$module/JEwECseLZdY"""),
                timeout = 15_000L
            )
        ).text
        ...
        tryParseJson<ArrayList<VidFastServers>>(res)?.filter { it.description?.contains("Original audio") == true }
            ?.amapIndexed { index, server ->
                val source =
                    app.get("${Drakor.vidfastAPI}/$module/Sdoi/${server.data}", referer = "${Drakor.vidfastAPI}/")
    ...
    suspend fun invokeWyzie(
        ...
        val url = if (season == null) {
            "${Drakor.wyzieAPI}/search?id=$tmdbId"
        } else {
            "${Drakor.wyzieAPI}/search?id=$tmdbId&season=$season&episode=$episode"
        }
    ...
    suspend fun invokeVixsrc(
        ...
        val type = if (season == null) "movie" else "tv"
        val url = if (season == null) {
            "${Drakor.vixsrcAPI}/$type/$tmdbId"
        } else {
            "${Drakor.vixsrcAPI}/$type/$tmdbId/$season/$episode"
        }
        ...
        listOf(
            ...
            VixsrcSource("Vixsrc [Beta]", video2, "${Drakor.mappleAPI}/"),
        ).map {
            callback.invoke(
                newExtractorLink(
                    it.name,
    ...
    suspend fun invokeSuperembed(
        ...
        val token = app.get("${Drakor.superembedAPI}/directstream.php?video_id=$tmdbId&tmdb=1$path").url.substringAfter(
            "?play="
        )
    ...
    suspend fun invokeVidrock(
        ...
        val type = if (season == null) "movie" else "tv"
        val url = "${Drakor.vidrockAPI}/$type/$tmdbId${if (type == "movie") "" else "/$season/$episode"}"
        val encryptData = VidrockHelper.encrypt(tmdbId, type, season, episode)

        app.get("${Drakor.vidrockAPI}/api/$type/$encryptData", referer = url).parsedSafe<LinkedHashMap<String, HashMap<String, String>>>()
            ?.map { source ->
                if (source.key == "source2") {
                    val json = app.get(source.value["url"] ?: return@map, referer = "${Drakor.vidrockAPI}/").text
                    ...
                    callback.invoke(
                        newExtractorLink(
                            ...
                            this.referer = "${Drakor.vidrockAPI}/"
                            this.headers = mapOf(
                                "Origin" to Drakor.vidrockAPI
                            )
                        }
                    )
                }
